...
function query (type, name, after) {
  return `query {
    rateLimit {
      cost
      remaining
    ...
    ${type ? 'user' : 'organization'}(login: "${name}") {
...
async function list (type) { // eslint-disable-line no-unused-vars
  const name = type ? document.getElementById('user').value : document.getElementById('organization').value;
  document.getElementById('loading').innerHTML = 'Loading';
  let after = '';
  let repos = [];
  do {
    await window.fetch('https://api.github.com/graphql', { method: 'POST', body: JSON.stringify({ query: query(type, name, after) }), headers: { Authorization: 'Bearer ' + bearer } })
      .then(res => {
        return res.json();
      })
      .then(res => {
        ...
        res = type ? res.data.user.repositories : res.data.organization.repositories;
  ...
  for (const repo of repos) {
    ...
    const fields = [element(repo.node.name, repo.node.url), text(repo.node.description),
      ...
      text(repo.node.owner.login.toLocaleLowerCase() === name.toLocaleLowerCase() ? (repo.node.isFork ? 'Fork' : 'Owner') : 'Collaborator'),
